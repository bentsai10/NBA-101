...
class FavoriteTeam{
    ...
    func saveData(completed: @escaping (Bool) -> ()){
        ...
        if self.documentID != "" {
            let ref = db.collection("faveTeams").document(self.documentID)
            ...
            ref = db.collection("faveTeams").addDocument(data: dataToSave) {error in
               if let error = error {
    ...
    func deleteData(completed: @escaping (Bool)->()){
        let db = Firestore.firestore()
        db.collection("faveTeams").document(self.documentID).delete(){error in
            if let error = error{
                print("error: could not properly delete favorite team")
                completed(false)
            }else{
                completed(true)
            }
